...
from make_new_t import exec_gjf##計算した点のxyzfileを出す
...
def main_process(args):
    ...
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['a','b','theta','z','A2','E','E_t','machine_type','status','file_name'])##いじる
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=1:
            ...
            Et1=float(E_list[0])
            E = 4*Et1
            df_E.loc[idx, ['E_t','E','status']] = [Et1,E,'Done']
        ...
        if len(dict_matrix)!=0:#終わりがまだ見えないなら
            for i in range(len(dict_matrix)):
                ...
                if not(alreadyCalculated):
                    ...
                    df_newline = pd.Series({**params_dict,'E':0.,'E_t':0.,'machine_type':machine_type,'status':'InProgress','file_name':file_name})
